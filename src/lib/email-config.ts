// ─────────────────────────────────────────────────────────────────────────────
// Email configuration — Resend client and template helpers
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * Returns a configured Resend instance.
 * Returns null if RESEND_API_KEY is not set (dev mode).
 */
...
/**
 * Returns a configured Resend instance, throwing if the API key is missing.
 * Use this when email sending is mandatory (not optional).
 */
...
/**
 * Wraps email body content in the standard GameBuddi email template.
 * Includes header, glowing divider, and footer with unsubscribe links.
 */
export function emailShell(title: string, bodyContent: string): string {
...
/**
 * Sends a verification code email. Falls back to console.log in dev mode.
 * Returns true if the email was sent, false if it was only logged.
 */
export async function sendVerificationCode(
  ...
  return sendEmail(to, `Your GameBuddi verification code: ${code}`, html)
}
...
/**
 * Sends a daily or weekly rankings email. Falls back to console.log in dev mode.
 */
export async function sendRankingsEmail(
  ...
  return sendEmail(to, `${label} Rankings - ${familyName} | GameBuddi`, html)
}
...
export async function sendTournamentInvite(
  ...
  return sendEmail(
    to,
    `You're invited: ${tournamentName} | GameBuddi`,
    html
  )
}
...
/**
 * Sends tournament results email.
 */
export async function sendTournamentResult(
  ...
  return sendEmail(to, `Results: ${tournamentName} | GameBuddi`, html)
}
...
/**
 * Sends an email via Resend if configured, otherwise logs to console.
 * Returns true if the email was actually sent via Resend.
 */
